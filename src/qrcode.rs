...
impl<'a> QrCode<'a> {
    ...
    /// Render this QR code as an SVG string.
    ///
    /// Each module is drawn as a `module_size` × `module_size` px square (default 4 px).
    /// A quiet zone of 4 modules is added on every side, as recommended by the QR Code
    /// specification.
    ///
    /// # Example
    ///
    ...
    pub fn to_svg_string(&self, module_size: i32) -> String {
        ...
        let size = self.size();
        let quiet_zone: i32 = 4; // 4-module quiet zone per QR spec
        let total_modules = size + 2 * quiet_zone;
        let px = total_modules * module_size;

        let mut rects = String::new();
        for y in 0..size {
            for x in 0..size {
                if self.get_module(x, y) {
                    let px_x = (x + quiet_zone) * module_size;
                    let px_y = (y + quiet_zone) * module_size;
                    rects.push_str(&format!(
                        r#"<rect x="{px_x}" y="{px_y}" width="{module_size}" height="{module_size}" fill="black"/>"#,
                    ));
                }
            }
        }
        ...
        format!(
            r#"<svg xmlns="http://www.w3.org/2000/svg" width="{px}" height="{px}" viewBox="0 0 {px} {px}"><rect width="{px}" height="{px}" fill="white"/>{rects}</svg>"#,
        )
    }

    /// Render this QR code as a grayscale image buffer.
    ///
    /// Each module is drawn as a `module_size` × `module_size` pixel square.
    /// A quiet zone of 4 modules is added on every side.
    ///
    /// This method is only available when the `image` feature is enabled.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    pub fn to_image(&self, module_size: u32) -> image::GrayImage {
        ...
        let px = total * module_size;

        let white = Luma([255u8]);
        let black = Luma([0u8]);
        let mut img = GrayImage::from_pixel(px, px, white);

        for y in 0..self.size() {
            for x in 0..self.size() {
                if self.get_module(x, y) {
                    let px_x = (x as u32 + quiet_zone) * module_size;
                    let px_y = (y as u32 + quiet_zone) * module_size;
                    for dx in 0..module_size {
                        for dy in 0..module_size {
                            img.put_pixel(px_x + dx, px_y + dy, black);
                        }
                    }
                }
            }
...
mod tests {
    ...
    fn test_qrcode_to_svg_string() {
        ...
        let mut tempbuffer = alloc::vec![0u8; Version::MAX.buffer_len()];
    ...
    fn test_qrcode_to_svg_string_custom_module_size() {
        ...
        let svg_small = qr.to_svg_string(2);
        let svg_large = qr.to_svg_string(8);
        // Larger module size should produce a larger SVG (larger width/height values)
        assert!(svg_large.len() > svg_small.len());
    }
